...
if((WP_DEBUG || WP_DEBUG == 'true' ) && (SCRIPT_DEBUG || SCRIPT_DEBUG == 'true' )){
	...
	wp_register_script('one-mwp-script', OCI_URL . "assets/js/mwp.js", array('one-mwp-jquery','one-mwp-wizard-script','media-upload','thickbox','wp-util'), $version );
	...
	wp_register_script('one-mwp-script', OCI_URL . 'assets/'.$resource_min_dir.'js/mwp'.$resource_extension.'.js', array('one-mwp-jquery','one-mwp-wizard-script','media-upload','thickbox','wp-util'), $version );
	$header_scripts = array(
		'one-mwp-style',
		'one-mwp-wizard-style',
		'thickbox'
